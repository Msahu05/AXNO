...
const CartFlyout = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          {latestItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <ShoppingBag className="h-12 w-12 text-[#8b8794] dark:text-white/40 mb-4" />
            ...
            <div className="space-y-4">
              {latestItems.map((item) => (
                <div
                  ...
                  className="flex gap-4 p-3 rounded-xl hover:bg-[#f6f1f8] dark:hover:bg-white/5 transition-colors"
                >
                  <img
        ...
        {/* Footer */}
        {latestItems.length > 0 && (
          <div className="p-4 sm:p-6 border-t border-[rgba(47,37,64,0.08)] dark:border-white/10 space-y-4">
            <div className="flex items-center justify-between">
              <span className="font-body text-sm text-[#8b8794] dark:text-white/70">Total:</span>
              <span className="font-heading text-lg font-semibold text-[#2f2540] dark:text-white">
                ₹{total}
              </span>
            </div>
            <Button
              onClick={handleViewCart}
              className="w-full rounded-[12px] bg-[#7b51f5] hover:bg-[#6a46d0] text-white font-body font-medium tracking-[0.5px]"
            >
              Go to Cart
            </Button>
          </div>
